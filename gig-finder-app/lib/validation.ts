...
export const bookingSchema = z.object({
    ...
    recordsQuantity: z.coerce.number().int().min(0).max(10).nullable().optional().default(0).transform(val => val ?? 0),
    recordsPrice: z.coerce.number().min(0).max(1000).nullable().optional().default(0).transform(val => val ?? 0),

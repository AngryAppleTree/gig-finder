...
async function checkBannermans() {
    ...
    try {
        console.log('🔍 Checking Bannerman\'s venues in PRODUCTION...\n');
        ...
        if (result.rows.length === 0) {
            console.log('❌ No Bannerman\'s venues found in production');
        } else {
            console.log(`Found ${result.rows.length} Bannerman's venue(s) in PRODUCTION:\n`);
            ...
            if (biggarVenue) {
                console.log('⚠️  ISSUE CONFIRMED: Found Bannerman\'s in Biggar');
                ...
                if (biggarVenue.event_count > 0) {
                    console.log('\n   ⚠️  WARNING: This venue has events!');
                    console.log('   You should reassign those events to the correct venue before deleting.');
                } else {
                    console.log('\n   ✅ Safe to delete - no events using this venue');
                }
            }
        }
